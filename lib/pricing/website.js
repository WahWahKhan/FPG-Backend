...
/**
 * Authoritative unit price for a Swell product, WITHOUT any stock check.
 * Used by TUBE360 to price tube material per metre (Tube360 never touches
 * Swell inventory, so a stock check would be wrong here).
 */
async function fetchSwellUnitPrice(productId) {
  ensureSwell();
  if (!productId) throw new PricingError('website: missing productId');
  let product;
  try {
    product = await swell.get('/products/{id}', { id: productId });
  } catch (err) {
    throw new PricingError(`website: failed to load product "${productId}": ${err.message}`);
  }
  if (!product || product.id === undefined) {
  ...
  return resolveUnitPrice(product, null);
}

module.exports = { priceWebsiteLine, fetchSwellUnitPrice };
