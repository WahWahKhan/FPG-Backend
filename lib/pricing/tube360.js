...
//   bending     = bends x bendRatePerBend (flat rate, linear in bend count)
...
function validateTube360Spec(spec, { requireBends = true } = {}) {
  ...
  if (angles.length > 0) {
    ...
    if (!isInt(r) || r !== entry.minClrMm) {
      throw new PricingError(
        `tube360: bend radius must be ${entry.minClrMm} mm (2 x the tube's OD) for ${entry.id}`
      );
    }
  }
  return { entry };
...
async function priceTube360Line(item, deps = {}) {
  ...
  const bendRate = r.bendRatePerBend;
  ...
  const bending = round2(bendCount * bendRate);
  ...
  return {
    ...
    breakdown: {
      ...
      bending: { count: bendCount, ratePerBend: bendRate, cost: bending },
      ends: {
