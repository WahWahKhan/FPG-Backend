...
export const generateInvoicePDF = (invoiceData: SupplierInvoiceData, captureId?: string): jsPDF => {
  const doc = new jsPDF();
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'bold');
  doc.text('Payment Status', margin + 3, footerY + 6);
  doc.setFont('helvetica', 'normal');
  doc.text('Paid via PayPal', margin + 3, footerY + 13);
  if (captureId) {
    doc.setFontSize(8);
    doc.text(`PayPal Capture ID: ${captureId}`, margin + 3, footerY + 20);
  }
