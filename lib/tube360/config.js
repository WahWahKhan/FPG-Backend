...
/** Bend complexity multiplier for a number of bends (tiers sorted ascending). */
function complexityMultiplier(bendCount) {
  let multiplier = 1;
  for (const tier of rates.bendComplexityTiers) {
    if (bendCount >= tier.minBends) multiplier = tier.multiplier;
  }
  return multiplier;
}
