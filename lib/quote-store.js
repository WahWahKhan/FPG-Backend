...
/**
 * Mark a saved cart as PURCHASED once its cart has been successfully paid.
 * Replaces the stored cart with a tombstone: the payable payload (items /
 * serverItems / quote) is dropped so a resume link can no longer rehydrate a
 * cart to pay again (Option A — single use), while a `purchased` flag lets
 * /resume return a friendly "already ordered" message instead of a generic
 * "expired" one (Option B). Idempotent and safe to call more than once.
 */
async function markSavedCartPurchased(token, internalOrderNumber) {
  if (!token) return;
  const existing = (await getRedis().get(savedCartKey(token))) || {};
  const tombstone = {
    purchased: true,
    purchasedAt: new Date().toISOString(),
    purchasedOrderNumber: internalOrderNumber || null,
    savedAt: existing.savedAt || null,
    version: existing.version || null,
    // items / serverItems / quote intentionally dropped — cart can't be re-paid.
  };
  await getRedis().set(savedCartKey(token), tombstone, { ex: SAVED_CART_TTL_SECONDS });
}
